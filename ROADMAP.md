...
- [x] Создать класс `Texture`: загрузка из `std::string_view`, освобождение в деструкторе
- [x] Запретить копирование `Texture`, реализовать move-конструктор с `noexcept`
- [x] Реализовать оператор присваивания `Texture` через copy-and-swap
- [x] Создать класс `Sound` по тому же паттерну (rule of five)
- [x] Создать класс `Music` с управлением воспроизведением (play/stop)
